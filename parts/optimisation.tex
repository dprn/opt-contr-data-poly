 ...
 If $\|A^\top \xi\|_\infty\le 1$ then $\langle A^\top\xi, x\rangle \ge -\|x\|_1$ and thus the infimum is attained for $x=0$, yielding $H(\xi)=-\langle\xi,y\rangle$. On the other hand, for $\|A^\top\xi\|_\infty>1$ let $i\in \llbracket1, m\rrbracket$ be the index such that $|(A^\top \xi)_i|=\|A^\top \xi\|$ and consider $x=-\operatorname{sgn}((A^\top \xi)_i)e_i$, so that $\langle A^\top\xi,x\rangle = -\|A^\top \xi\|_\infty$ and $\|x\|_1=1$. Thus, for any $\lambda>0$,
     ...
     \begin{proof}
        ...
        We start by observing that \eqref{eq:sum-grad-alpha} and the fact that $\sum_n\alpha_n=+\infty$ imply that $\liminf_{n\to +\infty} \|\nabla f(x_n)\|_2 =0$. Let $(x_{n_k})_k\subset (x_n)_n$ be a subsequence such that $\|\nabla f(x_{n_k})\|_2 \rightarrow 0$ as $k\to +\infty$.
        Since the sequence $(x_n)_n$ is bounded, up to extracting a further subsequence, we can assume that $x_{n_k}\rightarrow x_\infty$ for some $x_\infty\in \bar B(0,R)$. Then, by continuity of $\nabla f$ we have $\nabla f(x_\infty) =0$, which implies that $x_\infty = x^\star$ by uniqueness of the minimum.

        Observe now that the sequence $(f(x_n)_n)$ is non-increasing and bounded below by $f(x^\star)$. The Bolzano-Weierstrass Theorem then implies that $f(x_n)\rightarrow f_\infty \ge f(x^\star)$.
        But the fact that $x_{n_k}\rightarrow x^\star$ and the continuity of $f$ imply that 
        \begin{equation}
            f(x^\star)\le f_\infty = \lim_{n\to +\infty} f(x_{n}) = \lim_{k\to +\infty}f(x_{n_k}) =f(x^\star)
            \implies 
            f_\infty = f(x^\star).
        \end{equation}
        As a consequence, any subsequence $(x_{n_j})_j\subset(x_n)_n$ has a subsequence $(x_{n_{j_\ell}})_\ell$ which converges to some $z\in \bar B(0,R)$. Since $f(x_{n_{j_\ell}})\rightarrow f(x^\star)$, the continuity of $f$ then yelds $f(z)=f(x^\star)$, which by uniqueness of the minimum implies that $z=x^\star$.
        This shows that $x^\star$ is the only cluster point of $(x_n)_n$, concluding the proof.
     \end{proof}
