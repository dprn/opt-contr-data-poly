...
\prop[deep-identity]{}{
    ...
    The, for any $L\in \bbN$ and $\varepsilon>0$, there exists a neural network $\phi\in \mcN_d^d(\sigma;L,d)$ such that 
    \begin{equation}
...
\begin{proof}
   ...
   Let $s^\star\in \bbR$ be such that $\sigma$ is differentiable in a neighborhood of $s^\star$ and $\varsigma:=\sigma'(s^\star)\neq 0$.
    ...
    \end{equation}
    It is clear that $\phi_\lambda\in \mcN_d^d(\sigma;1,d)$.
...
\dfn[resnet]{Residual neural network}{
    Let $\sigma:\bbR\to \bbR$ be an activation function and let $d\in \bbN$.
    An $N$-hidden layer ResNet $\Phi_\theta:\bbR^{d}\to \bbR^{d}$ is identified by the set of parameters
    \begin{equation}
        \Theta = \left\{ (W^{(1)}_{\ell}, b^{(1)}_{\ell},W^{(2)}_{\ell}, b^{(2)}_{\ell})_{\ell=1}^{L+1} \mid W_{\ell}^{(i)}\in\bbR^{d\times d}, \, b_\ell^{(i)}\in \bbR^{d}, i =1,2\right\}.
    \end{equation}
    The corresponding function is then
    \begin{equation}
        \label{eq:ANN}
        \Phi_\theta(x) = \Phi^{L+1} \circ \Phi^L \circ\cdots\Phi^1(x),
    \end{equation}
    where, each layer $\Phi^\ell:\bbR^{d}\to \bbR^{d}$ is given by
    \begin{equation}
        \Phi^{\ell}(x) : x^{(\ell-1)} \in \bbR^{d} \mapsto x^{(\ell-1)} + \phi^\ell(x^{(\ell-1)}) \in \bbR^{d},   \qquad \ell\in \llbracket1,L\rrbracket,
    \end{equation} 
    for some standard $2$-layer feed-forward ANN $\phi^\ell:\bbR^{d}\to \bbR^{d}$ defined by   
    \begin{equation}
        \phi^\ell(x) = W_\ell^{(2)} \sigma(W_\ell^{(1)}x+b_\ell^{(1)}) + b_\ell^{(2)}.
    \end{equation}
}

The advantage of ResNets is that the skip connections (i.e., the addition of the input $x^{(\ell-1)}$ to the output of the layer $\phi^\ell$) help in mitigating issues like vanishing gradients during training evoked in Section~\ref{sec:issues-learning}. Indeed, adapting the backpropagation algorithm to ResNets, one can see that the gradients $\alpha^{(\ell)}$ defined in \eqref{eq:backprop} satisfy
\begin{equation}
...
The key observation in \cite{chenNeural2018,haberStable2018} is that ResNets can be interpreted as a forward Euler discretization of an ordinary differential equation (ODE). 
In particular, letting $u=(W^{(1)},b^{(1)},W^{(2)},b^{(2)})$ denote 
\begin{equation}
    F(x,u) :=  W^{(2)} \sigma(W^{(1)} x + b^{(1)}) + b^{(2)}.
\end{equation}
Then, fixing $\eta=1$, the ResNet defined by the parameters $\theta = \{(W_\ell^{(1)},b_\ell^{(1)},W_\ell^{(2)}, b_\ell^{(2)})\}_{\ell=1}^{L+1}$ can be rewritten as
\begin{equation}
...
\end{equation}
Thus, the ResNet can be seen as the Euler discretization of the ODE
\begin{equation}
    \dot x(t)= F(x(t),u(t)), \qquad t\in [0,L],
\end{equation}
with initial condition $x(0)=x\in \bbR^{d_0}$ and control $u$ defined as $u(t) = u_\ell$ for $t\in [(\ell-1)\eta, \ell \eta)$.
Rescaling the time variable as $t\mapsto t/L$ and letting $L\to +\infty$ we then obtain the continuous-time limit
\begin{equation}
    \dot x(t) = F(x(t),u(t)), \qquad t\in [0,1].
\end{equation}

This observation allows to interpret ResNets as \emph{neural ODEs}, i.e., ODEs whose dynamics is parametrized by a neural network.
...
\thm[neural-ODE]{Neural ODEs}{
    ...
    \begin{equation}
        x^{(\ell)}
        = x^{(\ell-1)} + \frac1L F(x^{(\ell-1)},u(\ell-1)), \qquad \ell\in \llbracket1,L\rrbracket.
    \end{equation}
    }
...
Since the control system given by the neural ODE is non-linear, we cannot apply directly the results of Part~\ref{part:control}.
...
% Moreover, for topological reasons, it is not possible to approximate arbitrary continuous functions with diffeomorphisms obtained as flows of ODEs.
...
% As a consequence of the above, the most we can expect is to approximate continuous functions $f:K\to \bbR^d$ that are homotopic to the identity.
The result we present below requires the following assumption on $f$.

\dfn[]{Monotone analytic homotopy}{
    A function $f:\bbR^d\to \bbR^d$ is monotone homotopic to the identity if there exists an analytic homotopy\footnote{That is, $h$ is analytic and such that $h(0,x)=x$ and $h(1,x)=f(x)$.} $h:[0,1]\times \bbR^d\to \bbR^d$  such that, for any $t\in [0,1]$, the map $h(t,\cdot)$ is monotone, i.e.,
    \begin{equation}
...
\thm[]{}{
    ...
    \begin{equation}
        \| f - \beta\circ \Phi_u\circ\alpha\|_K \le \varepsilon.
    \end{equation}
